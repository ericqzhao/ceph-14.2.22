...
enum CRUSHMAP_ITEM_TYPE {
  CRUSHMAP_NONE = 0,
  CRUSHMAP_DEVICES = 1,
  CRUSHMAP_BUCKETS = 2,
  CRUSHMAP_RULES = 3,
  CRUSHMAP_CHOOSE_ARGS = 4,
};
...
void usage()
{
  ...
  cout << "   --modify|-m txt-crush-map -ci crushmap-item modify crush map to source[default ci=3 only support to modify rules]\n";
  cout << "\n";
...
int main(int argc, const char **argv)
{
  ...
  bool compile = false;
  bool modify = false; // used for modify crushmap
  CRUSHMAP_ITEM_TYPE crushmap_item = CRUSHMAP_NONE;// used for modify crushmap
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
	...
	return EXIT_FAILURE;
      ...
      srcfn = val;
      compile = true;
    } else if (ceph_argparse_witharg(args, i, &val, "-m", "--modify", (char*)NULL)) {
      srcfn = val;
      modify = true;
    } else if (ceph_argparse_witharg(args, i, &val, "-ci", "--crushmap-item", (char*)NULL)) {
      crushmap_item = static_cast<CRUSHMAP_ITEM_TYPE>(atoi(val.c_str()));
  ...
  if (!check && !compile && !modify && !decompile && !build && !test && !reweight && !adjust && !tree && !dump &&
  ...
  */
  ...
  // used for modify crushmap with specified crushmap_item
  if (modify) {
    CrushWrapper crush2; // used for modify crushmap
    // encode used with crushmap txt
    crush2.create();
    // read the file
    ...
    CrushCompiler cc(crush2, cerr, verbose);
    if (unsafe_tunables)
    ...
    if (r < 0) 
      return EXIT_FAILURE;
    // item case
    switch (crushmap_item)
    {
      case CRUSHMAP_DEVICES:
        /* code */
        cerr << "crushmap item not support " << crushmap_item << std::endl;
        return EXIT_FAILURE;
        break;
      case CRUSHMAP_BUCKETS:
        /* code */
        cerr << "crushmap item not support " << crushmap_item << std::endl;
        return EXIT_FAILURE;
        break;
      case CRUSHMAP_RULES:
        // free old_rule mem
        for (unsigned int i=0;i<crush.get_crush_map()->max_rules;i++) {
          kfree(crush.get_crush_map()->rules[i]);
        }
        kfree(crush.get_crush_map()->rules);

        crush.get_crush_map()->max_rules = cc.GetCrushWrapper().get_crush_map()->max_rules;
        crush.get_crush_map()->rules = cc.GetCrushWrapper().get_crush_map()->rules;
        crush.rule_name_map = cc.GetCrushWrapper().rule_name_map;

        // init rules to NULL to avoid mem repeated release
        cc.GetCrushWrapper().get_crush_map()->max_rules = 0;
        cc.GetCrushWrapper().get_crush_map()->rules = NULL;
        break;
      case CRUSHMAP_CHOOSE_ARGS:
        /* code */
        cerr << "crushmap item not support " << crushmap_item << std::endl;
        return EXIT_FAILURE;
        break;     
      default:
        cerr << "unable to decode crushmap item " << crushmap_item << std::endl;
        return EXIT_FAILURE;
    }
    modified = true;
  }
